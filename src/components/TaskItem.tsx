...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  const handleGenerateAISuggestions = async () => {
    ...
    try {
      // Generate task suggestions based on the task title and description
      ...
      if (tasks && tasks.length > 0) {
        ...
        setEditedTask(prev => ({
          ...prev,
          implementation_tips: [
            ...(prev.implementation_tips || []),
            ...(suggestions.implementation_tips || [])
          ],
          potential_challenges: [
            ...(prev.potential_challenges || []),
  ...
  const handleRemoveResource = (index: number, type: 'resource' | 'learning' | 'tool') => {
    setEditedTask(prev => {
      switch (type) {
        case 'resource':
          return {
                ...
                <div className="space-y-2">
                  {editedTask.learning_resources.map((resource, index) => (
                    <div key={index} className="flex items-center justify-between bg-white p-2 rounded border border-gray-200">
                      ...
                      <button
                        onClick={() => handleRemoveResource(index, 'learning')}
                        className="ml-2 p-1 text-gray-400 hover:text-gray-500"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                ...
                <div className="space-y-2">
                  {editedTask.tools.map((tool, index) => (
                    <div key={index} className="flex items-center justify-between bg-white p-2 rounded border border-gray-200">
                      ...
                      <button
                        onClick={() => handleRemoveResource(index, 'tool')}
                        className="ml-2 p-1 text-gray-400 hover:text-gray-500"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
